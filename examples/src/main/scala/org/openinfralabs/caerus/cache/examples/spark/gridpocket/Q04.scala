...
/**
 * Query 4 in Spark for GridPocket dataset. Obtain the consumption of meters in Paris for a month.
 * @param spark          Spark session used to execute the query.
 * @param schemaProvider Schema provider in Spark for GridPocket dataset.
 */
case class Q04(spark: SparkSession, schemaProvider: GridPocketSchemaProvider) extends GridPocketQuery {
  override def execute(args: String*): DataFrame = {
    ...
    loadDF
      ...
      .select(col("day"), col("vid"), col("cons"))
