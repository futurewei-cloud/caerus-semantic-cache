...
/**
 * Query 2 in Spark for GridPocket dataset. In order to display a cluster map, obtain each meter with its info
 * (city, id, etc.).
 * @param spark Spark session used to execute the query.
 * @param schemaProvider Schema provider in Spark for GridPocket dataset.
 */
case class Q02(spark: SparkSession, schemaProvider: GridPocketSchemaProvider) extends GridPocketQuery {
  override def execute(args: String*): DataFrame = {
    ...
    loadDF
      ...
      .agg(
        ...
        functions.first(col("city")).as("city"),
      ...
      .select(col("month"), col("vid"), col("cons"), col("city"), col("lat"), col("lng"), col("region"))
      .orderBy(col("vid"))
  }
}
